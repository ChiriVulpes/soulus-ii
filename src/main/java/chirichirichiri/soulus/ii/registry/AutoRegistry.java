...
import chirichirichiri.soulus.ii.utility.function.chaining.Splat;
import chirichirichiri.soulus.ii.utility.translation.annotation.Annotations;
...
public abstract class AutoRegistry<T extends IForgeRegistryEntry<T>, A extends Annotation> {
	...
	protected void register (final IForgeRegistry<T> registry) {
		Annotations.getAnnotatedFields(getRegistrationAnnotationClass())
	...
	private boolean filter (final Field field) throws IllegalAccessException {
		...
		SoulusII.LOGGER.error("Non-registration field '" + field.getName() + "' in '" + field.getDeclaringClass().getName() + "' annotated with @'" + getRegistrationAnnotationClass().getName() + "'");
		return false;
	}
