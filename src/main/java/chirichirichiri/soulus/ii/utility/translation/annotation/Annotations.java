package chirichirichiri.soulus.ii.utility.translation.annotation;
...
import chirichirichiri.soulus.ii.utility.function.chaining.Graceful;
...
public class Annotations {
	...
	private static ModFileScanData getScanData () {
		if (scanData == null)
			scanData = SoulusII.INFO.getOwningFile()
				.getFile()
				.getScanResult();
	...
	public static Stream<Class<?>> getAnnotatedClasses (final Class<? extends Annotation> annotationClass) {
		return getAnnotations(annotationClass)
			...
			.map(annotation -> annotation.getClassType().getClassName())
			.map(Graceful.map(Class::forName));
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static Map<Class<?>, Stream<Field>> getAnnotatedFieldsByClass (final Class<? extends Annotation> annotationClass) {
		return getAnnotations(annotationClass)
			.filter(a -> a.getTargetType() == ElementType.FIELD)
			.collect(Collectors.groupingBy(Graceful.map(annotation -> Class.forName(annotation.getClassType().getClassName()))))
			.entrySet()
			.stream()
	...
	public static Stream<Field> getAnnotatedFields (final Class<? extends Annotation> annotationClass) {
		return getAnnotatedFieldsByClass(annotationClass)
			.values()
			.stream()
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static Map<Class<?>, Stream<Method>> getAnnotatedMethods (final Class<? extends Annotation> annotationClass) {
		return getAnnotations(annotationClass)
			.filter(a -> a.getTargetType() == ElementType.METHOD)
			.map(annotation -> annotation.getClassType().getClassName())
			.map(Graceful.map(Class::forName))
			.distinct()
			.collect(Collectors.toMap(Function.identity(), cls -> Arrays.stream(cls.getDeclaredMethods())
