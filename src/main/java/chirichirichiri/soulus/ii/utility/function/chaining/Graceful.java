package chirichirichiri.soulus.ii.utility.function.chaining;
...
public class Graceful {
	...
	public static <E extends Exception> void ignoring (final Runnable runnable, final Consumer<E> handler, final Class<E>... exceptionClasses) {
		try {
			runnable.run();
	...
	@SafeVarargs
	public static <T, R, E extends Exception> Function<T, R> map (final FunctionThrows<? super T, ? extends R, E> mapper, final Function<E, R> handler, final Class<E>... exceptionClasses) {
	...
	@SuppressWarnings("unchecked")
	public static <T, R, E extends Exception> Function<T, R> map (final FunctionThrows<? super T, ? extends R, E> mapper, final Class<E>... exceptionClasses) {
	...
	@SafeVarargs
	public static <T, E extends Exception> Predicate<T> filter (final PredicateThrows<? super T, E> mapper, final Consumer<E> handler, final Class<E>... exceptionClasses) {
	...
	@SuppressWarnings("unchecked")
	public static <T, R, E extends Exception> Predicate<T> filter (final PredicateThrows<? super T, E> predicate, final Class<E>... exceptionClasses) {
	...
	public Graceful run (final Runnable runnable) {
		try {
			...
			final Consumer<Exception> handler = handlers.entrySet()
				.stream()
				.filter(h -> h.getKey().isInstance(e))
				.findFirst()
